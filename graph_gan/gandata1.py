...
from graph_gan.generator import Generator, MLP_G, ModelEma
from graph_gan.model_data1 import Model
...
micro_dis = pd.read_csv("../mirco-dis.csv", header=None)
...
m_sim = pd.read_csv("../data/microbe_features.txt", header=None, index_col=None ,sep='\t').values
d_sim = pd.read_csv("../data/disease_features.txt", header=None, index_col=None ,sep='\t').values
...
def train_gan_d(model,D_model,G_model,train_data,optimizer_d,optimizer,edge_label_index,edge_label,cosine_schedule):
    ...
    #errD_real = F.binary_cross_entropy_with_logits(errD_real_out, torch.tensor(1.).to(device))
    errD_fake = D_model(fake_hidden.detach())
    #errD_fake = F.binary_cross_entropy_with_logits(errD_fake_out,torch.tensor(1.).to(device))
    ...
    #                                          method='sparse')
    # all_edge_label_index = torch.cat(
    #     [train_data['micro', 'cause', 'disease'].edge_label_index, neg_edge_label_index],
    #     dim=-1,
    # )
    ...
    #     train_data['micro', 'cause', 'disease'].edge_label.new_zeros(neg_edge_label_index.size(1))
    # ], dim=0)
    # out = model.decode(real_hidden_,all_edge_label_index)
    # loss = F.binary_cross_entropy_with_logits(out,all_edge_label)
    #
    # all_loss =  loss + 0.1 * errD
    # print("loss:",loss)
    #
    # all_loss.backward()
    # optimizer.step()
...
with open("../logs/data1/gan/gan_auc.csv","w") as csvfile:
...
def mytest():
    ...
    for epoch in range(200):
        ...
        #loss, _, _ = train(model, optimizer, train_data,cosine_schedule)
        ...
        with torch.no_grad():
            ...
            with open("../logs/data1/gan/gan_auc.csv", "a+") as csvfile:
                writer = csv.writer(csvfile)
