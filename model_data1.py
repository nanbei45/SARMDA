...
class GAT(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.conv3 = GATv2Conv((-1, -1), hidden_channels, add_self_loops=False, heads=1)
        self.lin3 = Linear(hidden_channels, hidden_channels)
        self.conv4 = GATv2Conv((-1, -1), hidden_channels, add_self_loops=False, heads=1)
...
class Model(torch.nn.Module):
    def __init__(self,hidden_channels,data):
        super().__init__()
        self.micro_lin = torch.nn.Linear(292,hidden_channels)
        self.micro_emb = torch.nn.Embedding(data["micro"].num_nodes,hidden_channels)
        self.disease_lin = torch.nn.Linear(39, hidden_channels)
        self.disease_emb = torch.nn.Embedding(data["disease"].num_nodes, hidden_channels)
        self.gnn = GCN(hidden_channels=hidden_channels)
